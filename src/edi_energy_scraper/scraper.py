...
class EdiEnergyScraper:
    ...
    def filter_by_format_version(
        self, documents: list[Document], format_version: EdifactFormatVersion
    ) -> list[Document]:
        """
        Filters a list of documents by their EDIFACT format version.

        Args:
            documents: List of Document objects to filter
            format_version: The EdifactFormatVersion to filter by

        Returns:
            A list of Document objects that match the specified format version
        """
        return [doc for doc in documents if get_edifact_format_version(doc.validFrom) == format_version]
    ...
    async def mirror(self) -> None:
        ...
        # Use the new filter method instead of inline filtering
        fv2404_documents = self.filter_by_format_version(all_metadata, EdifactFormatVersion.FV2404)
        for document in fv2404_documents:
